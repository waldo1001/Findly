...
Once the Developer Program membership is active and you have the **Team ID**:
- `web/join/.well-known/apple-app-site-association.json` → replace `CHANGE-ME-TEAMID` with
  the real Team ID (appID becomes `TEAMID.com.findly.ios`).
- `mobile/ios/Findly/Findly.entitlements` → uncomment Associated Domains, set
  `applinks:<new JOIN_LINK_HOST>`.
- Register App ID `com.findly.ios`, upload APNs key to Firebase (step 3), apply for the
  Location Push entitlement, create the App Store Connect app as **Findly**.
