...
- Scan the diff and every newly tracked file for secret material:
  ...
  **Documented exception — Firebase *Web* app config inlined in page source.** An `AIza…` hit is **not** a finding when all of these hold: it is a Firebase **Web** app key (the accompanying `appId` contains `:web:`), it is inlined in a page that cannot function without it (a browser page has no separate config file to gitignore, unlike `google-services.json`), and **no other credential-shaped value accompanies it** in the diff. Such keys identify a project; they do not authenticate access — authorization is enforced server-side by Firebase **ID-token** verification (001 §2.2), so the key grants nothing on its own. This carve-out exists because the grep will otherwise re-flag `web/join/delete-account.html` on every future touch and force each reviewer to re-derive the same judgment from memory. It does **not** extend to server keys, service-account JSON, or an `AIza…` value appearing anywhere a config file *could* have been gitignored instead. Operator-side hardening (HTTP-referrer + API restrictions on the key in the Google Cloud console) is recommended and tracked separately — its absence is not a code finding.
...
- Intent/deep-link inputs validated before use (invite codes, `findly://group-join` codes, 007 https-link fragments — pure whitelist normalization before any network call; wrong host/path ignored, never mis-routed).
