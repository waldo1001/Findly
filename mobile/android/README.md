...
## Status (A1 — foundation, done; A2 — feature screens, done; A12 — real map, done)
...
**Waived (H1 — Azure/Firebase provisioning, still `human`/pending):** no real `google-services.json` (stays gitignored/absent), no real backend base URL, no real Firebase project, no real Maps API key value (the SDK wiring itself is done, A12). Everything that needs these is stubbed behind an interface — `AuthProvider`, `PushTokenProvider`, `AppConfig`'s `BASE_URL`/`FIREBASE_PROJECT_ID`/`MAPS_API_KEY` — and documented in specs/003 §13. Wiring the real thing is a same-interface swap, not a redesign.
