...
3. Parent-only controls (non-parents see 1–2 only, read-only — the 003 §12.1 client-side `isParent` gate is unchanged):
   - **Tracking** `FindlySwitchRow` — commits immediately (unchanged). *(Corrected 2026-08-26: this section originally said `FindlyToggleRow`, a component that has never existed on either platform. The contracted name in 003 §4.3 / 004 §2.3 is `FindlySwitchRow` — a `FindlyListRow` with a trailing themed `Switch`, added by A2 for exactly this control. Surfaced by A35.)*
