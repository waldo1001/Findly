...
- **(Amended 2026-08-26, row A37.) A *confirmed* auth failure is not an inconclusive probe.** `AUTH_MISSING_TOKEN`, `AUTH_INVALID_TOKEN`, `AUTH_TOKEN_EXPIRED` and `AUTH_FORBIDDEN` (001 §10) MUST route to **Sign-in**, clearing the local session, and MUST NOT fail open to the Family Map. Rendering the app shell for a caller the backend has confirmed is unauthorized is a state-machine bug: every screen's data call then 401s individually, so the user sees a working-looking app that fetches nothing — a dead session that reads as a broken app. **Distinguishing confirmed from transient:** the probe makes one attempt with no retry, so "transient" means the transport failed (timeout, connection error) or the server returned 5xx — i.e. **no error code was received**. A response carrying one of the four auth codes above is confirmed by definition, and the `transient 401` in the bullet above means only "a 401 that arrived without a decodable error envelope". Clients MUST branch on the typed error code, never on the HTTP status alone.
...
- The roster lives in a **`FindlyBottomSheet`** with three detents, **sized to their content** (amended 2026-08-27, row I46 — this replaces the handoff's fixed **186pt/dp** and **440pt/dp** values, which assumed a fuller roster than a real family has and left dead space below the content at both detents on a 4-member family; **the contents of each detent stay normative, their heights do not**): **minimized** (grabber, "Family" title, summary line, `Locate now` for the selected member when one is selected, avatar stack) — exactly the height that content needs and no more, since this detent's whole purpose is to maximise the map; **standard** (header + member list) — the measured height of the header plus however many member rows exist, **capped at the platform "large" detent** so a large family cannot exceed it; **expanded** (platform "large" detent; list scrolls). A detent MUST NOT render empty space below its own content. The map stays fully rendered and interactive behind the sheet at every detent — **"maximize the map" is the minimized detent**, not a separate mode. *(Amended 2026-08-26, row I40: on iOS this requires `presentationBackgroundInteraction`, which is **iOS 16.4+**, while the app's deployment target is 16.0. On **iOS 16.0–16.3 the map is rendered but not interactive** while the sheet holds focus; there is no 16.0-compatible substitute for the API. Clients MUST guard the call with `if #available` and degrade to a plain sheet rather than dropping the detents. Android is unaffected.)* Dragging between detents MUST NOT unmount the list or re-create view models (the I16 `@StateObject` ownership rule; on iOS the detents map to `.presentationDetents`).
...
**Padding model (amended 2026-08-26, row I39 — normative).** The 64 dp/pt above is a **fixed screen-space margin**, not a proportional inflation of the bounding box. Both clients MUST convert it to device pixels at the render boundary and hand it to the platform's bounds-fitting API (Android: `LocalDensity` → `CameraUpdateFactory.newLatLngBounds`). A percentage-based inflation is **not** conformant: the two models diverge in opposite directions at the extremes — for a geographically tight family a fixed margin dominates the frame, while for the two-country family this section's own problem statement names, a fixed margin shrinks to nearly nothing where a 30 % inflation would keep a wide border. Pinning only the *value* let both clients conform while framing the same family at materially different zoom, which is the defect this amendment closes.

**Occlusion model (added 2026-09-07, row I49 — normative).** The bounds-fit MUST be computed against the map area **not covered by the roster sheet at its current detent**, not against the full map viewport. Both clients render the map full-bleed behind the sheet, which is correct, but it makes the raw viewport the wrong target: at the standard detent the sheet hides roughly the bottom third and when expanded roughly half, so fitting the full frame places members underneath it. This is observable in the shipped app and in the store screenshots — with a geographically spread four-member family, two of the four markers land behind the sheet after a fit-all, which means the one action whose entire purpose is *"show me everyone"* demonstrably does not.

Concretely: subtract the sheet's current height from the usable viewport height, fit the bounds into what remains, and measure the 64 dp/pt margin from **that** reduced area's edges. The margin is unchanged and still a fixed screen-space value; only the rectangle it is measured against changes. This is a **viewport inset, not a padding change** — increasing the padding instead would shrink the framing on all four sides and still not guarantee the bottom edge clears the sheet.

Applies to both clients (Android `MapCamera.kt` / `GoogleMapRenderer`, iOS `MapCameraPolicy` / `MapRegion(fitting:viewSizePt:)`). They occlude identically, so a fix on one platform only would reintroduce exactly the cross-client divergence the padding model above was written to close.

**A detent change MUST NOT move the camera.** Dragging the sheet up after a fit-all therefore hides members again, and the user must tap fit-all a second time to reframe them. That is the deliberate choice, not an oversight: the rule below permits camera movement on exactly three triggers, and a detent drag is a gesture about the *sheet*, not about the map — animating the map under the user's own finger would be more disorienting than the stale framing it fixes, and it would make the camera move without the user ever having asked it to. The fit-all uses whatever detent is current **at the moment it is invoked**. Revisit only if the second tap proves to be a real annoyance in use.
...
3. Parent-only controls (non-parents see 1–2 only, read-only — the 003 §12.1 client-side `isParent` gate is unchanged):
   - **Tracking** `FindlySwitchRow` (Android) / `FindlyToggleRow` (iOS) — commits immediately (unchanged). *(Corrected 2026-08-26 by A35: this section originally said `FindlyToggleRow` unqualified, and Android genuinely has no such component — its contracted name is `FindlySwitchRow` (003 §4.3, added by A2). **Further corrected 2026-08-26 by I36**: A35's fix overgeneralized to "never existed on either platform" and "004 §2.3 is `FindlySwitchRow`" — both false for iOS. `specs/004-ios-client.md` §2.3 unambiguously and currently names `FindlyToggleRow` (added by I2, the same PR wave as A2), and `DesignSystem/Components/FindlyToggleRow.swift` is a real, shipped, already-in-use component (this exact Devices screen's Tracking row, pre-dating this batch). The two platforms have simply never shared one name for this control — 003/004 §2.1's "identical names" goal has one standing, unreconciled exception here. iOS is NOT renaming `FindlyToggleRow`; only Android's contracted name was ever wrong in this section. Tracked as backlog row **I41**.)*
   - **Sync interval — a `FindlyDropdownField`** (label "Sync interval", current value shown closed; open presents the 7 allowed values of 001 §1.4 — `5 min, 10 min, 15 min, 30 min, 1 hour, 2 hours, 1 day` — via the platform-native menu: exposed dropdown menu on Android, `Menu`/`Picker` presentation on iOS). Selecting a value **commits immediately** (`PATCH /devices/{id}`), no Save button. Values below `features.limits.minSyncIntervalMinutes` render disabled with the limit as the reason — read from `features`, never hardcoded (000 §Subscription-ready). *(Amended 2026-09-06, 009 §1.3 / 000 §D19)* the menu groups the values under two captions — **"Live"** for 5–30 min ("keeps Findly running in the background; on Android a persistent notification is shown") and **"Battery saver"** for 1 hour–1 day ("reports when the phone wakes up; positions may be hours old") — and the closed field shows the group name after the value (e.g. `15 min · Live`).
...
4. **This-device controls — Android, shown regardless of role (added 2026-09-06; 009 §3.2).** On the card representing **the device the app is running on**, and only that card: a **"Battery settings"** action opening the battery-optimisation prompt or, once the prompt has been answered, the system battery page; plus the vendor-specific link (dontkillmyapp.com) on the OEMs 009 §3.2 lists. These are **not** parent-only and MUST NOT sit inside bullet 3's gate: they configure the local OS's treatment of this installation, not a family setting, and 009 §3.2's requirement is unconditional. A child's phone is a non-parent account and is exactly the device whose manufacturer is most likely to kill the presence service, so gating these behind `isParent` would hide them from the user who needs them. *(This corrects the 2026-09-06 amendment that first added the sentence inside bullet 3 — found by A41's code review as a direct 009/010 conflict.)*
5. Errors from this card's mutations render **on this card**, not pooled at the top of the list (iOS's current shared top-of-list `lastActionError` placement is retired).
