...
| `displayName` | 1–30 characters after normalization. **Normalized server-side at every write (added 2026-09-07, B29):** strip Unicode bidirectional controls (U+202A–U+202E, U+2066–U+2069), newlines, and other C0/C1 control characters, then trim; a value that normalizes to empty → `400 VALIDATION_FAILED`. The **character set is deliberately not restricted** — accents, non-Latin scripts and emoji are all legitimate in real names, and narrowing it would exclude real users. Only characters that are never legitimate in a name are removed, and they are removed **at write time** so stored data is clean and every reader inherits it. This matters because `displayName` reaches two notification surfaces as untrusted text: `data.requestedByName` on the §8.1 locate push, which the Android client renders on-device, and the **server-composed** §8.2 `GEOFENCE_EVENT` title, which iOS displays directly from `aps.alert.title` with no client rendering step — so a client-side sanitiser cannot cover it. Unsanitized, a member could embed a right-to-left override in their own name once and have every later alert on every other member's phone render reversed or truncated text, defeating the notification's purpose as a transparency signal (009 §9). Applies to every path that accepts one: §3.1, §3.4, §3.5, §12.1 and §12.6. *(Corrected 2026-09-07: this list originally also cited §12.7, which is **Rotate join code** and carries no `displayName` at all — found by B29's implementing agent, which flagged the citation rather than inventing a field to match it.)* |
