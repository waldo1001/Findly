...
| `displayName` | 1–30 characters after normalization. **Normalized server-side at every write (added 2026-09-07, B29):** strip Unicode bidirectional controls (U+202A–U+202E, U+2066–U+2069), newlines, and other C0/C1 control characters, then trim; a value that normalizes to empty → `400 VALIDATION_FAILED`. The **character set is deliberately not restricted** — accents, non-Latin scripts and emoji are all legitimate in real names, and narrowing it would exclude real users. Only characters that are never legitimate in a name are removed, and they are removed **at write time** so stored data is clean and every reader inherits it. This matters because `displayName` reaches **two server-composed notification titles**, both of which iOS displays directly from `aps.alert.title` with no client rendering step, so a client-side sanitiser cannot cover either: the §8.1 locate title (`"<requestedByName> is locating you"` — server-composed on iOS since the 2026-09-06 §8.1 amendment; Android additionally renders its own copy on-device from `data.requestedByName`) and the §8.2 `GEOFENCE_EVENT` title. *(Framing corrected 2026-09-07: this row originally described §8.1 as Android-rendered only, which stopped being true when §8.1 became a visible alert push.)*

**Composing a title MUST NOT depend on stored data having been normalized.** Normalization is a write-time rule, so any value stored before it — or by a future path that bypasses the schema — is still hostile. Both compose points therefore re-normalize, and because a fully hostile stored value normalizes to the **empty string**, both MUST substitute the placeholder **`"Someone"`** rather than emit a title with a missing name (`" arrived at Home"`, `" is locating you"`). The same applies to an empty `geofenceName`, which uses **`"a place"`**. A user id MUST NOT be used as the fallback: it is opaque to the reader and would expose an internal identifier to every other family member. Unsanitized, a member could embed a right-to-left override in their own name once and have every later alert on every other member's phone render reversed or truncated text, defeating the notification's purpose as a transparency signal (009 §9). Applies to every path that accepts one: §3.1, §3.4, §3.5, §12.1 and §12.6. *(Corrected 2026-09-07: this list originally also cited §12.7, which is **Rotate join code** and carries no `displayName` at all — found by B29's implementing agent, which flagged the citation rather than inventing a field to match it.)* |
| `geofenceName` | 1–50 characters (§7.2's existing bound — **corrected 2026-09-07 from the 1–40 this row first claimed**, which matched neither §7.2's prose nor the shipped validation and would have **rejected already-valid names of 41–50 characters**; caught by B29's implementing agent, which implemented what it was told and flagged the conflict rather than silently reconciling it). Normalized exactly as `displayName` above and for the same reason — it is interpolated into the same server-composed §8.2 title. |
...
- Locate: instant `lastKnown`; ordered target resolution (§6.1) incl. no-devices → 404 and all-paused → 403; coalescing (excluded from quota); quota; poll authorization; lazy expiry. **Amended 2026-09-06:** `expiresAt = createdAt + 180 s` and `createdAt` on both the create and poll responses; the §6.3 grace window on **both** sides of its boundary — a fulfil at or inside `expiresAt + 10 min` returns `fulfilled` with `late: true` and writes `fixJson`/`fulfilledAt`, one past it still returns `410` with the fix stored; the `expired → fulfilled` transition; `late: false` for a fulfil before `expiresAt`, read back from storage rather than recomputed on poll; fulfil idempotency on the same `fixId` and the different-`fixId` rule (stored to last-known + history, `fixJson` unchanged, no `410`) **including beyond the grace window**; a push **transport** failure (a thrown send *and* a non-throwing `error` outcome — FCM 5xx) creates `pushFailed` **without** `pushInvalid`, while an `invalidToken` outcome creates `pushFailed` **with** it.
