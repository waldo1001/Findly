...
**Enforcement.** `CoroutineScopeExceptionHandlerConventionTest` (`app/src/test/java/com/findly/android/`) mechanically enforces the first rule (handler presence + class-name-only logging) on every `.kt` file under `app/src/main` on every `./gradlew test` run, following the same source-text structural-test pattern `MainActivityInsetsStructureTest`/`WindowThemeDayNightTest` already established for framework glue this module has no UI-test harness to exercise directly. A custom Detekt rule was considered and rejected for this: it needs its own Gradle subproject wired against `detekt-api`, a rule-testing harness, and ongoing version-pairing upkeep against this module's Kotlin/AGP versions — real, continuing tooling weight for a one-developer project that has already deliberately declined a Compose UI test harness rather than carry it (backlog A38). The structural test reuses tooling already in the module (JUnit, `java.io.File`) for a one-file, no-new-dependency cost.
