...
### 3.1 CoroutineScope convention (mandatory — A42; corrected by two A42 review-fix rounds — a best-effort heuristic, not a proof)
...
- A bare mention of `CoroutineExceptionHandler` in a constructor-parameter or property type annotation (e.g. `class Foo(private val handler: CoroutineExceptionHandler)`, as opposed to a function's return type, which *is* excluded) is still counted as a declaration needing its own verifiable body and can false-fail code that never constructs anything at that position.
- String-literal masking assumes ordinary Kotlin string syntax (`"..."`, `"""..."""`, `\"` escapes, `$identifier`/`${expr}` templates); it is a hand-written scanner, not the Kotlin lexer, and an exotic-enough literal could in principle still defeat it.
