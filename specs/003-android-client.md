...
### 3.1 CoroutineScope convention (mandatory — A42; corrected by the A42 review-fix round — measured coverage, not the tool's own claim)
...
- The second rule above (cancellation pass-through on a touched `catch (e: Exception)`/`catch (t: Throwable)`): reliably distinguishing "this `try` block contains a suspension point" from source text alone needs real type/suspend-context analysis; a text-scan heuristic wrong in either direction (flagging a legitimate synchronous catch, or missing a real one behind an intermediate non-suspend-looking call) would erode trust in the check faster than it prevents bugs. Documented review convention only.
- Handler-by-reference resolution is same-file only, as stated above — a handler `val` re-exported from another file (a companion object in a different class, a top-level `val` in another file) will not resolve and the site will fail the check even if the reference is, in fact, a real handler; move the construction site and its handler into the same file rather than splitting them.
- Source-text scanning in general: a string literal that happens to contain the text `CoroutineScope(`, `MainScope()`, or `GlobalScope` would falsely trip the checks (comments are stripped before scanning; string contents are not). Not observed in this codebase; noted because it is a property of the approach, not something the test tries to rule out.

The second rule (cancellation pass-through) is **not** mechanically enforced — reliably distinguishing "this `catch (e: Exception)`'s `try` block contains a suspension point" from source text alone needs real type/suspend-context analysis, and a text-scan heuristic wrong in either direction (flagging a legitimate synchronous catch, or missing a real one wrapped in an intermediate non-suspend-looking call) would erode trust in the check faster than it prevents bugs. This one stays a documented review convention: when you touch a `catch (e: Exception)`/`catch (t: Throwable)`, check whether its `try` block can suspend, and if so, add `catch (e: CancellationException) { throw e }` before it.
