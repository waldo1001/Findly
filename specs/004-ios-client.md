...
| `Theme.onSurfaceMuted` | `#3A4160` / `#98A1BD` | Muted/secondary text (`FindlyListRow` subtitles, `FindlyTextField` placeholder/label, `FindlyEmptyState`/`FindlyErrorState` body, `FindlySectionHeader`, `PermissionBannerView` dismiss icon) — `onSurface` at ~70% given as exact per-theme hex rather than a computed alpha (I30 — see below, light value corrected past HANDOFF.md's own literal) | 9.08:1 vs `surface`, 8.01:1 vs `surfaceVariant` | 7.43:1 vs `surface`, 6.49:1 vs `surfaceVariant` |
...
**I30 (2026-08-06) — `Theme.onSurfaceMuted` light darkened `#4E5675` → `#3A4160`, a deliberate design change, not a HANDOFF.md arithmetic correction like the two above.** A user reported the light theme's text as barely readable on a real **Android** device; the same person explicitly said **iOS looked fine** with the identical value. Investigating the Android report sampled the actual rendered pixels off a Settings screenshot and found `#4E5675` on `#F2F4FB` exactly — no alpha, no platform default leaking in — so it was not a bug: it is HANDOFF.md's own specified value (*"subtitle 13/400 in `onSurface` at 70% (use `#4E5675` light)"*) rendering faithfully at **6.56:1**, which passes AA but is thin for a low-chroma blue-grey used at 13pt/13sp for subtitles and section headers — AA is a floor, not a target. **iOS is changing for cross-platform consistency, not because it was observed failing — a future reader should not conclude iOS had the same bug.** The plausible reason for the perceptual difference is that SF renders heavier than Roboto at these sizes, or that the screens looked at differed; neither is asserted as fact. New value measures **9.08:1** vs `surface` and **8.01:1** vs `surfaceVariant` (independently verified), still clearly a secondary tier against `onSurface` `#10142A`'s 16.54:1. Dark `#98A1BD` is unchanged — already 7.43:1 / 6.49:1, materially better than light's old value, which is probably why dark never drew the complaint on either platform. Same change and rationale as Android's A30 (specs/003-android-client.md); the `PermissionBannerView` dismiss-icon pairing (`ColorContrastPairings.swift`, "PermissionBannerView dismissIcon/surfaceVariant") also reads this token, so its real (threshold-checked, not exact-pinned) ratio moves too.
...
> **`swift test` runs under AppKit, not UIKit — decided 2026-08-26 (rows I38, I41).** `FindlyKitTests` compiles against the **macOS host toolchain**, so every `#if os(iOS)` block is skipped and I18's `SwiftUIRenderingHarness` falls back to `NSHostingController`. A UIKit-specific defect is therefore invisible to `swift test`, and this is **not** hypothetical: I37's `InvitePasteControl` redeclared `UIPasteConfigurationSupporting` (which `UIResponder` already provides), compiled clean under `swift test`, and was caught only by `xcodebuild`. **Rule: any task touching a `UIViewRepresentable` or other UIKit bridge MUST run `xcodebuild` against a simulator destination and MUST NOT rely on `swift test` alone.** Adding an `xcodebuild test` step was considered and declined (row I38) — a slower `ios` pipeline is not worth it for a one-developer project; the rule above is the mitigation. **Component-name parity (row I41):** 003 §4.3 and this section aim at identical component names across the clients, with one standing, deliberate exception — Android's `FindlySwitchRow` is iOS's **`FindlyToggleRow`**. Both predate the 010 batch, both are shipped at multiple call sites (device tracking *and* geofence notify flags), and renaming either is a cross-cutting change for cosmetic parity. It is documented rather than fixed; do not treat it as a bug.

`FindlyButton` (primary/secondary style), `FindlyCard`, `FindlyListRow`, `StatusChip` (e.g. online/stale/paused), `MapMarkerBubble`, `FindlyNavBar`, `EmptyStateView`, `LoadingStateView`, `ErrorStateView`, plus two I2 additions needed by the feature-screen forms: `FindlyTextField` (a labeled single-line text input) and `FindlyToggleRow` (a label/subtitle row with a trailing themed toggle, e.g. device pause/`trackingEnabled`, geofence `notifyOnEnter`/`notifyOnExit`), plus three **010-shell additions (2026-08-26)** mirroring 003 §4.3 per the Bucket-B rule: `FindlyNavDrawer` (010 §1.2 — a coordinator-driven overlay; iOS has no drawer primitive, which stays an implementation detail behind the component), `FindlyBottomSheet` (the three-detent roster sheet, 010 §3.1 — `.presentationDetents`-backed), and `FindlyDropdownField` (labeled single-select dropdown — device sync interval, 010 §4.2). Each:
...
> **Amendment (2026-08-26):** the navigation *shell* is now normative in **[010](010-app-shell-and-screen-ux.md)** — the Family Map replaces the `.home` hub as the authenticated root (behind a `FindlyNavDrawer`), the Home button-list screen is retired, and `PROFILE_NOT_FOUND`/`FAMILY_NOT_FOUND` on any screen load routes to the 010 §2.2 Onboarding root. Every rule in this section (stack ownership, idempotent pushes, root-replacement resets, modal back-action clearing, `popTo`) remains authoritative — `showHome()`'s reset semantics apply verbatim with the map as the reset target. Where this section's screen references conflict with 010, **010 wins**.
...
> **Amendment (2026-08-26):** this section's mechanics (neutral `.launching` root, resolve after the UI exists, never re-prompt SMS) are unchanged, but the *destination* of a restored session is now the **010 §1.1 launch-resolution table** — Family Map for a profiled+familied user, the Onboarding variants otherwise — not `.home`.
...
> The on-device runtime contract — the three opportunistic iOS triggers (`BGAppRefreshTask`, significant-location-change, geofence transitions) and the `× 0.8` elapsed-time rule, the durable Core Data/SQLite queue replacing §6's in-memory store, push routing, region-monitoring lifecycle, and the When-In-Use → Always staging — is normative in **[009](009-device-runtime.md)** (tasks I10–I12). The scaffolding described below is what those tasks replace. **Amended 2026-09-06 (009 §1.3/§3.4/§5.1, 000 §D19):** `SystemLocationProvider` must set `allowsBackgroundLocationUpdates = true` (+ `showsBackgroundLocationIndicator = false`) whenever authorization is Always — without it every background `requestLocation()` times out; for intervals ≤ 30 it runs a standing kilometre-accuracy **presence** session with a cadence timer; SLC and visit monitoring start only with Always; the BG-refresh expiration handler completes the task; `LOCATE_REQUEST` now arrives as alert + `content-available` and the `AppDelegate` handler wraps the capture in `beginBackgroundTask` and completes the fetch handler once the fulfil request is sent; notification authorization is actually requested on first sign-in (tasks I50–I52).
