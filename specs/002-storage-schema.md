...
- **"Created lazily by its first write" is a client-side guarantee for tables, not innate Azure behavior (B23).** Table Storage does not auto-create a table on write — a real account provisioned with zero tables 500s `TableNotFound` on the very first write otherwise. **This sentence used to add "the way this codebase's blob store self-heals via create-if-not-exists on append (§3.2)", and that comparison was false in a way that cost a live P0 (B25, 2026-08-06).** §3.2's create-if-not-exists is at the **blob** level, not the **container** level: an append creates the missing day-blob, but nothing anywhere creates the missing container, and a write into a container that does not exist fails with `ContainerNotFound`. The two are one word apart and read identically when skimming, which is exactly how B23 came to fix tables while deliberately exempting containers, and how `POST /locations` and `PUT /geofences` 500'd in production from first deploy until the containers were created by hand. **Azurite creates containers implicitly on write, so no test in this repo — unit or Azurite-integration — can observe the difference.** Provisioning must create `config`, `history` and `events` explicitly (`docs/azure-setup.md`). `createTableClient` (`backend/src/adapters/tables/tableClientFactory.ts`) is the single chokepoint every table adapter goes through: it ensures the table exists (an idempotent create-if-not-exists) before that table's first operation in the process, then caches the confirmation for the process's lifetime so later operations pay no extra round trip.
