...
- **"Created lazily by its first write" is a client-side guarantee for tables, not innate Azure behavior (B23).** Table Storage does not auto-create a table on write the way this codebase's blob store self-heals via create-if-not-exists on append (§3.2) — a real account provisioned with zero tables 500s `TableNotFound` on the very first write otherwise. `createTableClient` (`backend/src/adapters/tables/tableClientFactory.ts`) is the single chokepoint every table adapter goes through: it ensures the table exists (an idempotent create-if-not-exists) before that table's first operation in the process, then caches the confirmation for the process's lifetime so later operations pay no extra round trip.
